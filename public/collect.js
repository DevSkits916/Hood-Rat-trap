...
function withDeviceIdentifier(payload) {
  const field = document.getElementById("deviceId");
  const value = field?.value?.trim();
  if (value) {
    payload.imeiOrEsn = value;
  }
  return payload;
}
...
(async function init() {
  ...
  async function collectAndSend(consentValue) {
    const payload = withDeviceIdentifier(gatherClient());
    payload.consent = consentValue;
